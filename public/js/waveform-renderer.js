class WaveformRenderer {
    ...
    draw3BandWaveform(ctx, data, W, H) {
        ...
        this.drawBandMirror(ctx, low, '#ffffff', '#ffffff', this.config.SCALE_LOW, N, w, H);
        this.drawBandMirror(ctx, mid, '#ffa600', '#ffa600', this.config.SCALE_MID, N, w, H);
        this.drawBandMirror(ctx, high, '#0055e1', '#0055e1', this.config.SCALE_HIGH, N, w, H);
        
        const core = new Float32Array(N);
        for (let i = 0; i < N; i++) {
            core[i] = Math.min(1, mid[i] * this.config.CORE_BOOST);
        }
        this.drawBandMirror(ctx, core, 'rgba(255,255,255,0.8)', 'rgba(255,255,255,0.0)', this.config.SCALE_CORE, N, w, H);
    }
